	...
	<div class="content">
		...
		<div class="innerOuter">
			...
			<!-- 
				...
				- 일반적인 텍스트 기반의 폼 데이터 전송시 사용하는 인코딩 방식.
				- form 태그 내부의 모든 데이터를 key=value 형태로 인코딩하여 전달.
				- 단, 전달할 데이터 중 file 형태의 데이터가 포함된 경우 전송이 불가능.
				
				2. multipart/form-data
				- 파일 업로드시 반드시 사용해야하는 enctype.
				- 텍스트 데이터와 바이너리 형태의 데이터(파일)를 개별적인 파트로 나누어 각각 따로 전송하는 방식.
				- 서버는 멀티파트 데이터로 요청한 데이터에 대해서는 별도의 파싱 과정을 거쳐야만 브라우저에서 전달한 파람값들에 대해 접근이 가능.
				- Spring 기반 서버에서는 MultipartResolver를 등록하여 HttpServletRequest 객체를
				  MultipartHttpServletRequest로 변환해야 파람에 접근 가능.
			-->
			<form:form modelAttribute="b" action="${contextPath }/board/insert/${boardCode }" id="enrollForm" method="post"
