...
import com.newlearn.playground.security.model.vo.MemberExt;
...
public class SecurityController {
	...
	// 생성자 방식 의존성 주입(생성자가 현재 클래스에 1개라면 @AutoWired 생략가능)
	 ...
	 * 
	 * 처리순서
	 * 1) 클라이언트의 요청 파라미터를 Argument Resolver가 커맨드 객체 필드에 바인딩.
	...
	public void initBinder(WebDataBinder binder) {
		...
		// 타입변환
		// 문자열 형태의 날짜값을 Date로 변환
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyMMdd"); // 생년월일
		dateFormat.setLenient(false); // yyMMdd 형식이 아닌 경우 에러를 발생을 끔
	...
	public String register(
			...
			// BindingResult
			// - 유효성검사 결과를 저장하는 객체.
			// - forward시 자동으로 jsp에게 전달되며, form태그 내부에 에러 내용을 바인딩 할 때 사용.
		...
		// 유효성 검사 통과시 비밀번호 정보는 암호화 하여 회원가입 진행
		String encryptPassword = passwordEncoder.encode(member.getUserPwd());
