...
import com.newlearn.playground.chat.model.service.ChatService;
...
@RequestMapping("/chat")
@Slf4j
@SessionAttributes({"chatRoomNo"}) // model 영역에 추가하는 key 값이 chatRoomNo인 데이터를 HttpSession(sessionScope)에 저장.
public class ChatController {
	...
	public String openChatRoom(
		...
		/*
		 * 업무 로직
		 * 1. 유효성 검사(생략. 구현시 initbinder, validator 추가해서 할 것)
	...
	public String joinChatRoom(
		...
		if (list == null) {
			throw new RuntimeException("채팅방 참여 실패.");
		}
