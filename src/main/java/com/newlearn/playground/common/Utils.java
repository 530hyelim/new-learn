...
public class Utils {
	...
	/*
	 * XSS(Cross Site Script) 공격을 방지하기 위한 메서드
	 * - 스크립트 삽입 공격.
	 * - 사용자가 <script> 태그를 게시글에 작성하여 클라이언트가 게시글 클릭시 script에 지정한 코드가 실행되도록 유도하는 공격.
	 * - 위 내용을 그대로 DB에 저장 후 브라우저에 렌더링하면 문제가 발생할 수 있으므로 태그가 아닌 기본 문자열로 인식할 수 있게끔 html 내부 entity로 변환.
	 */
